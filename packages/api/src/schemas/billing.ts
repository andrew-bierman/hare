...
export const PlanSchema = z.object({
	...
	name: z.string().min(1).max(50).describe('Display name of the plan'),
	description: z.string().min(1).max(200).describe('Brief description of the plan'),
	price: z.number().min(0).nullable().describe('Monthly price in USD (null for custom pricing)'),
	priceId: z.string().min(1).max(100).nullable().describe('Stripe price ID for this plan'),
...
export const CheckoutResponseSchema = z.object({
	url: z.string().url().describe('Stripe checkout session URL to redirect the user'),
	sessionId: z.string().min(1).max(255).describe('Stripe checkout session ID'),
})
...
export const PortalResponseSchema = z.object({
	url: z.string().url().describe('Stripe customer portal URL'),
})
...
export const BillingUsageSchema = z.object({
	...
	agentsLimit: z.number().int().min(-1).describe('Maximum agents allowed (-1 for unlimited)'),
	messagesUsed: z.number().int().min(0).describe('Messages sent this billing period'),
	messagesLimit: z.number().int().min(-1).describe('Maximum messages allowed (-1 for unlimited)'),
})
...
export const BillingStatusSchema = z.object({
	...
	planName: z.string().min(1).max(50).describe('Display name of the current plan'),
...
export const PaymentHistoryItemSchema = z.object({
	id: z.string().min(1).max(255).describe('Stripe charge ID'),
	amount: z.number().min(0).describe('Amount in dollars (converted from cents)'),
	currency: z.string().length(3).toUpperCase().describe('Currency code in uppercase'),
	status: z.string().min(1).max(50).describe('Payment status from Stripe'),
	description: z.string().max(500).nullable().describe('Payment description'),
	createdAt: z.string().datetime().describe('When the payment was created'),
	invoiceUrl: z.string().url().nullable().describe('URL to view/download the receipt'),
})
...
export const PaymentHistoryResponseSchema = z.object({
	...
	hasMore: z.boolean().describe('Whether more payments exist beyond this page'),
})
