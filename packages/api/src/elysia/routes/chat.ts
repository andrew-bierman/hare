...
export const chatRoutes = new Elysia({ name: 'chat-routes' })
	...
	.get('/agents/:id/conversations', async ({ db, params }) => {
		const agentId = params.id
	...
	.get('/agents/:id/conversations/search', async ({ db, params, query }) => {
		const agentId = params.id
		...
		const conditions = [eq(conversations.agentId, agentId), like(messages.content, `%${searchQuery}%`)]
	...
	.get('/conversations/:id/messages', async ({ db, params }) => {
