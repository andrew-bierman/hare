...
import { adminPlugin, writePlugin, type AuthUserContext, type WorkspaceInfo } from '../context'
...
async function findAgent(id: string, workspaceId: string, db: Database) {
...
export const agentRoutes = new Elysia({ prefix: '/agents', name: 'agent-routes' })
	...
	.get('/', async ({ db, workspaceId }) => {
		...
		return { agents: results.map((agent) => serializeAgent(agent, toolIdsByAgentId.get(agent.id) || [])) }
	}, { writeAccess: true })
	...
	// Create agent
	.post('/', async (ctx) => {
		const { db, workspaceId, user, body, request } = ctx
		const [agent] = await db
			.insert(agents)
			.values({
				workspaceId,
		...
		if (body.toolIds && body.toolIds.length > 0) {
			const customToolIds = body.toolIds.filter((id: string) => !id.startsWith('system-'))
			if (customToolIds.length > 0) {
				await db.insert(agentTools).values(
					customToolIds.map((toolId: string) => ({ agentId: agent.id, toolId })),
				)
			}
		}
	...
	.patch('/:id', async (ctx) => {
		...
		const updateData: Partial<typeof agents.$inferInsert> = {
			...
			...(body.systemToolsEnabled !== undefined && { systemToolsEnabled: body.systemToolsEnabled }),
			...(body.status !== undefined && { status: body.status }),
		}

		const [agent] = await db.update(agents).set(updateData).where(eq(agents.id, params.id)).returning()
		...
		if (body.toolIds !== undefined) {
			...
			const customToolIds = body.toolIds.filter((toolId: string) => !toolId.startsWith('system-'))
			if (customToolIds.length > 0) {
				await db.insert(agentTools).values(
					customToolIds.map((toolId: string) => ({ agentId: params.id, toolId })),
				)
			}
		}
	...
	.post('/preview', async ({ body }) => {
		...
		if (!instructions || instructions.trim().length === 0) {
			warnings.push({ field: 'instructions', message: 'Agent should have instructions for best results', type: 'warning' })
		}
		if (!model) {
	...
	.post('/:id/clone', async (ctx) => {
		...
		if (sourceToolIds.length > 0) {
			...
			if (customToolIds.length > 0) {
				await db.insert(agentTools).values(
					customToolIds.map((toolId) => ({ agentId: clonedAgent.id, toolId })),
				)
			}
		}
	...
	.post('/:id/rollback', async (ctx) => {
		...
		if (restoredToolIds.length > 0) {
			...
			if (customToolIds.length > 0) {
				await db.insert(agentTools).values(
					customToolIds.map((toolId) => ({ agentId: params.id, toolId })),
				)
			}
		}
