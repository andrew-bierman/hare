	...
	return crypto.subtle.deriveKey(
		{
			...
			iterations: config.security.encryption.PBKDF2_ITERATIONS,
			hash: 'SHA-256',
		},
		passwordKey,
		{ name: 'AES-GCM', length: config.security.encryption.AES_KEY_LENGTH },
		false,
		['encrypt', 'decrypt'],
	)
}
...
export async function encryptData(options: EncryptDataOptions): Promise<string> {
	...
	const iv = crypto.getRandomValues(new Uint8Array(config.security.encryption.IV_SIZE))

	// Generate random salt for key derivation
	const salt = crypto.getRandomValues(new Uint8Array(config.security.encryption.SALT_SIZE))
...
export async function decryptData(options: DecryptDataOptions): Promise<string> {
	...
	const salt = combined.slice(0, config.security.encryption.SALT_SIZE)
	const iv = combined.slice(
		config.security.encryption.SALT_SIZE,
		config.security.encryption.SALT_SIZE + config.security.encryption.IV_SIZE,
	)
...
export function generateSecret(options: GenerateSecretOptions = {}): string {
	const { length = config.security.encryption.DEFAULT_SECRET_LENGTH } = options
