...
import { workspaceMembers } from '@hare/api-client'
...
export function useWorkspaceInvitationsQuery(workspaceId: string | undefined) {
	return useQuery({
		...
		queryFn: async () => {
			const res = await workspaceMembers[':id'].invites.$get({
				param: { id: workspaceId! },
			})
			if (!res.ok) throw new Error('Request failed')
			return res.json()
...
export function useSendInvitationMutation() {
	...
	return useMutation({
		mutationFn: async ({ workspaceId, data }: { workspaceId: string; data: SendInvitationInput }) => {
			const res = await workspaceMembers[':id'].invites.$post({
				param: { id: workspaceId },
				json: data,
			})
			if (!res.ok) throw new Error('Request failed')
...
export function useRevokeInvitationMutation() {
	...
	return useMutation({
		mutationFn: async ({ workspaceId, inviteId }: { workspaceId: string; inviteId: string }) => {
			const res = await workspaceMembers[':id'].invites[':inviteId'].$delete({
...
export function useRemoveMemberMutation() {
	...
	return useMutation({
		mutationFn: async ({ workspaceId, userId }: { workspaceId: string; userId: string }) => {
			const res = await workspaceMembers[':id'].members[':userId'].$delete({
...
export function useUpdateMemberRoleMutation() {
	...
	return useMutation({
		mutationFn: async ({
			...
			const res = await workspaceMembers[':id'].members[':userId'].$patch({
				param: { id: workspaceId, userId },
				json: { role },
			})
			if (!res.ok) throw new Error('Request failed')
