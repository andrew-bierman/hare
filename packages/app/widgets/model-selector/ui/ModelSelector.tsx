...
function ModelBadge({ tier, type }: { tier: string; type: 'speed' | 'cost' }) {
	const labelConfig = type === 'speed' ? config.models.labels.speed : config.models.labels.cost
	const { label, color } = labelConfig[tier as keyof typeof labelConfig] ?? { label: tier, color: '' }
