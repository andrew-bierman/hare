...
export function ToolDetailPage({
	...
	// Build test input from form state (for custom tools using fields state)
	const buildTestInput = (): Record<string, unknown> => {
		...
		for (const [key, value] of Object.entries(testInput)) {
			...
			if (field) {
				...
				// For system tools, try to parse based on the schema
				const inputSchema = tool?.inputSchema as InputSchema | undefined
				const prop = inputSchema?.properties?.[key]
				if (prop) {
					result[key] = parseDefaultValue(value, prop.type as FieldType)
				} else {
					result[key] = value
				}
			}
	...
	// Render test input fields based on input schema
	const renderTestInputFields = () => {
		const inputSchema = tool?.inputSchema as InputSchema | undefined
		const isSystemTool = tool?.isSystem

		// For system tools, use the tool's input schema directly
		if (isSystemTool && inputSchema?.properties) {
			const schemaProperties = Object.entries(inputSchema.properties)
			const requiredFields = inputSchema.required || []

			if (schemaProperties.length === 0) {
				return (
					<div className="text-center py-6 text-muted-foreground">
						<p>This tool has no input parameters.</p>
					</div>
				)
			}
			...
			return (
				<div className="space-y-4">
					{schemaProperties.map(([fieldName, prop]) => {
						const isRequired = requiredFields.includes(fieldName)
						const fieldType = prop.type as FieldType

						return (
							<div key={fieldName} className="space-y-2">
								<Label htmlFor={`test-${fieldName}`} className="flex items-center gap-1">
									{fieldName}
								...
								{renderFieldInput(fieldName, fieldType, prop)}
							</div>
						)
					})}
				</div>
		...
		// For custom tools, use the fields state
		if (fields.length === 0) {
			return (
				<div className="text-center py-6 text-muted-foreground">
					<p>No input parameters defined.</p>
					<p className="text-sm">Add parameters in the Settings tab to test with inputs.</p>
				</div>
			)
		}
				...
				{fields
					...
					.map((field) => (
						<div key={field.id} className="space-y-2">
							<Label htmlFor={`test-${field.name}`} className="flex items-center gap-1">
								{field.name}
	...
	const renderFieldInput = (
		fieldName: string,
		fieldType: FieldType,
		prop: { enum?: string[]; default?: unknown },
	) => {
		...
		if (prop.enum && prop.enum.length > 0) {
			return (
				<Select
					...
					onValueChange={(value) => setTestInput({ ...testInput, [fieldName]: value })}
				>
					<SelectTrigger id={`test-${fieldName}`}>
			...
			case 'boolean':
				return (
					<div className="flex items-center gap-2">
						<Checkbox
							id={`test-${fieldName}`}
							checked={testInput[fieldName] === 'true'}
							onCheckedChange={(checked) =>
								setTestInput({ ...testInput, [fieldName]: checked ? 'true' : 'false' })
							}
						/>
						<Label htmlFor={`test-${fieldName}`} className="text-sm font-normal">
							{testInput[fieldName] === 'true' ? 'true' : 'false'}
						</Label>
					</div>
				)
			...
			case 'number':
				return (
					<Input
						id={`test-${fieldName}`}
						type="number"
						placeholder={prop.default !== undefined ? String(prop.default) : `Enter ${fieldName}`}
						value={testInput[fieldName] || ''}
						onChange={(e: ChangeEvent<HTMLInputElement>) =>
							setTestInput({ ...testInput, [fieldName]: e.target.value })
						}
					/>
				)
								...
								<CardContent className="space-y-4">
									...
									<div className="space-y-2">
										...
										<Input
											...
											value={responsePath}
											onChange={(e: ChangeEvent<HTMLInputElement>) =>
												setResponsePath(e.target.value)
											}
										/>
					...
					<div className="grid gap-4 lg:grid-cols-2">
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>
									{isSystemTool
										? 'Enter test values for tool parameters'
										: 'Enter test values based on your input schema'}
								</CardDescription>
							</CardHeader>
							<CardContent className="space-y-4">
								{/* Generate form fields from input schema */}
								{renderTestInputFields()}

								<Button className="w-full" onClick={handleTest} disabled={testTool.isPending}>
									{testTool.isPending ? (
										<>
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>View the response from your tool execution</CardDescription>
							</CardHeader>
							<CardContent>
								{!testResult ? (
									<div className="text-center py-12 text-muted-foreground">
										...
										<p className="text-sm">Execute a test to see the response.</p>
									</div>
								) : (
									<div className="space-y-4">
										...
										<div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
											<div className="flex items-center gap-2">
												...
												<span
													className={`font-medium ${testResult.success ? 'text-green-500' : 'text-destructive'}`}
												>
										...
										{/* Response Data */}
										{(testResult.data !== undefined || testResult.result !== undefined) && (
											<div className="space-y-2">
												<Label className="text-sm font-medium">Response Data</Label>
												<pre className="text-xs bg-muted p-3 rounded-lg overflow-auto max-h-64 border">
													{JSON.stringify(testResult.data ?? testResult.result, null, 2)}
												</pre>
											</div>
										)}
										...
										<div className="border-t pt-4">
											...
											{showRequestDetails && testResult.requestDetails && (
												<div className="mt-3 space-y-3 text-sm">
													<div className="flex items-center gap-2">
														<span className="text-muted-foreground min-w-16">URL:</span>
														<code className="text-xs bg-muted px-2 py-1 rounded">
															{testResult.requestDetails.url}
														</code>
													</div>
													<div className="flex items-center gap-2">
														<span className="text-muted-foreground min-w-16">Method:</span>
														<Badge variant="outline">{testResult.requestDetails.method}</Badge>
													</div>
					...
					<Card>
						...
						<CardContent className="space-y-2 text-sm text-muted-foreground">
							<ul className="list-disc list-inside space-y-1">
								<li>Test executions are logged but don't count toward usage metrics.</li>
								<li>For HTTP tools, the test will make a real request to the configured URL.</li>
								<li>Input values will replace {'{{variable}}'} placeholders in the request.</li>
								<li>Check the Request Details section to debug your configuration.</li>
							</ul>
						</CardContent>
					</Card>
				</TabsContent>
