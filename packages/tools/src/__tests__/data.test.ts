	...
	describe('scrapeTool', () => {
		...
		describe('execution', () => {
			...
			it('extracts links from page', async () => {
				...
				expect(result.data?.links[0].href).toBe('https://test.com')
			})
			...
			it('extracts meta tags', async () => {
				...
				expect(result.success).toBe(true)
				expect(result.data?.meta?.title).toBe('Test Page')
	...
	describe('regexTool', () => {
		...
		describe('execution - replace', () => {
			it('replaces matches', async () => {
				...
				expect(result.success).toBe(true)
				expect(result.data?.result).toBe('hello there')
			})
		})
		...
		describe('execution - extract', () => {
			it('extracts captured groups', async () => {
				...
				expect(result.success).toBe(true)
				expect(result.data?.extracted).toHaveLength(2)
				expect(result.data?.extracted[0].user).toBe('user')
				expect(result.data?.extracted[0].domain).toBe('example.com')
			})
		})
	...
	describe('cryptoTool', () => {
		...
		describe('execution', () => {
			...
			it('generates random bytes', async () => {
				...
				expect(result.data?.bytes).toBe(16)
			})
			...
			it('encrypts and decrypts data', async () => {
				...
				expect(decryptResult.data?.decrypted).toBe('secret message')
			})
		})
	})
	...
	describe('csvTool', () => {
		...
		describe('execution - parse', () => {
				...
				const result = await csvTool.execute(
					{
						operation: 'parse',
						data: 'name,age\nJohn,30\nJane,25',
						options: { headers: true, delimiter: ',' },
					},
					context,
				)
				expect(result.success).toBe(true)
				expect(result.data?.data).toHaveLength(2)
				expect(result.data?.data[0].name).toBe('John')
			...
			it('handles quoted values', async () => {
				...
				expect(result.success).toBe(true)
				expect(result.data?.data[0].bio).toBe('Hello, World')
			})
		})
		...
		describe('execution - stringify', () => {
			it('converts objects to CSV', async () => {
				...
				expect(result.data?.csv).toContain('John,30')
			})
			...
			it('escapes special characters', async () => {
				...
				expect(result.success).toBe(true)
				expect(result.data?.csv).toContain('"John, Jr."')
			})
		})
	})
