...
test.describe('Authentication', () => {
	...
	test('should have email, name, and password fields on sign-up', async ({ page }: { page: Page }) => {
		await page.goto('/sign-up')
	...
	test('should show validation error for empty form submission', async ({ page }: { page: Page }) => {
		await page.goto('/sign-in')
...
test.describe('API Auth Endpoints', () => {
	test('should return 401 for unauthenticated workspace requests', async ({ request }: { request: APIRequestContext }) => {
		const response = await request.get('/api/workspaces')
		expect(response.status()).toBe(401)
	})

	test('should return 401 for unauthenticated agent requests', async ({ request }: { request: APIRequestContext }) => {
		const response = await request.get('/api/agents?workspaceId=test')
		expect(response.status()).toBe(401)
	})

	test('should return 401 for unauthenticated tool requests', async ({ request }: { request: APIRequestContext }) => {
		const response = await request.get('/api/tools?workspaceId=test')
		expect(response.status()).toBe(401)
	})

	test('should return 401 for unauthenticated usage requests', async ({ request }: { request: APIRequestContext }) => {
