...
export const ToolSchema = z
  .object({
    ...
    description: z
      .string()
      .openapi({ example: "Make HTTP requests to external APIs" }),
    type: ToolTypeSchema,
    ...
    code: z
      ...
      .openapi({
        example:
          "export default async function(input) { return fetch(input.url) }",
      }),
    isSystem: z.boolean().openapi({ example: true }),
    createdAt: z
      .string()
      .datetime()
      .openapi({ example: "2024-12-01T00:00:00Z" }),
    updatedAt: z
      .string()
      .datetime()
      .openapi({ example: "2024-12-01T00:00:00Z" }),
...
export const CreateToolSchema = z
  .object({
    ...
    description: z
      .string()
      .min(1)
      .openapi({ example: "A custom tool for my agent" }),
    type: ToolTypeSchema,
    inputSchema: JsonSchemaSchema.openapi({
...
export const UpdateToolSchema =
  CreateToolSchema.partial().openapi("UpdateTool");
