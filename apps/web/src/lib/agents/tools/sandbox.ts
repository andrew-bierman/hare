...
function hasSandbox(env: CloudflareEnv): env is CloudflareEnvWithSandbox {
  return (
    "SANDBOX" in env && (env as CloudflareEnvWithSandbox).SANDBOX !== undefined
  );
}
...
async function executeWithCloudfareSandbox<T>(
  ...
  try {
    ...
    return failure(
      `Sandbox error: ${error instanceof Error ? error.message : "Unknown"}`,
    );
  }
}
...
export const codeValidateTool = createTool({
  id: "code_validate",
  description:
    "Validate code before execution. Checks for syntax and potential issues.",
  inputSchema: z.object({
  ...
  execute: async (input, _context) => {
    ...
    if (language === "javascript") {
      ...
      try {
        ...
        issues.push(
          `Syntax error: ${e instanceof Error ? e.message : "Unknown"}`,
        );
      }
    }
...
export const sandboxFileTool = createTool({
  id: "sandbox_file",
  description:
    "Read or write files in the Cloudflare Sandbox container. Requires SANDBOX binding.",
  inputSchema: z.object({
    operation: z
  ...
  execute: async (input, context) => {
    if (!hasSandbox(context.env)) {
      return failure(
        "Sandbox not available. Add SANDBOX binding to wrangler.toml.",
      );
    }
